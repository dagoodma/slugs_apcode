 ...
 */
static  uint16_t mavlink_msg_scaled_pressure_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, uint64_t usec, float press_abs, float press_diff, int16_t temperature)
{
	uint16_t i = 0;
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static  uint16_t mavlink_msg_scaled_pressure_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, uint64_t usec, float press_abs, float press_diff, int16_t temperature)
{
	uint16_t i = 0;
	msg->msgid = MAVLINK_MSG_ID_SCALED_PRESSURE;
...
static  void mavlink_msg_scaled_pressure_send(mavlink_channel_t chan, uint64_t usec, float press_abs, float press_diff, int16_t temperature)
{
 ...
 */
static  uint64_t mavlink_msg_scaled_pressure_get_usec(const mavlink_message_t* msg)
{
	generic_64bit r;
	r.b[7] = (msg->payload)[0];
	r.b[6] = (msg->payload)[1];
 ...
 */
static  float mavlink_msg_scaled_pressure_get_press_abs(const mavlink_message_t* msg)
{
	generic_32bit r;
 ...
 */
static  float mavlink_msg_scaled_pressure_get_press_diff(const mavlink_message_t* msg)
{
	generic_32bit r;
 ...
 */
static  int16_t mavlink_msg_scaled_pressure_get_temperature(const mavlink_message_t* msg)
{
	generic_16bit r;
